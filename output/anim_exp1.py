"""
  anim_exp1.py - plot cells on substrate, with fixed colormap ranges (rf. fix_cmap, vmax_substrate)
...
vmin_substrate = 0.
vmax_substrate = 1.e-5

kdx = 1
vmax_substrate = float(sys.argv[kdx])
...
scale_radius = 1.0
#if (len(sys.argv) == 12):
# 0 0 0 3000 0 3000 0 3000 0 3000 0
if (True):
  use_defaults = False
  show_nucleus = 0
  current_idx = 0
...
#    xvec = grid2D[0,:]
    ...
    levels = MaxNLocator(nbins=30).tick_values(vmin_substrate, vmax_substrate)
...
#    my_plot = plt.contourf(xvec,xvec,M[field_idx,:].reshape(N,N), num_contours, cmap='viridis') #'viridis'
    ...
    if cbar == None:
      ...
      print('colorbar vmin_substrate,vmax_substrate = ',vmin_substrate,vmax_substrate)
      cbar = plt.colorbar(my_plot, cax=cbar.ax, boundaries=np.arange(vmin_substrate, vmax_substrate, 1.0))
...
#    print("keys=",child.attrib.keys())
    ...
    if child.text and "agents" in child.text:
        svals = child.text.split()
        num_cells = svals[0]
...
# to plot the grid, comment out the following lines
#  line_collection = LineCollection(grid_lines, color="gray", linewidths=0.1)
#  ax.add_collection(line_collection)
#  ax.set_xlim(xs[0], xs[-1])
#  ax.set_ylim(ys[0], ys[-1])

  # to specify region of interest
